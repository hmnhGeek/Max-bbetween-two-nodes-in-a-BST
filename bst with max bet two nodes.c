...
void find_max(bst *node, int start, int stop){
    if(node != NULL){
        find_max(node -> left, start, stop);
        if((node -> data > start && node -> data < stop) && (large < node -> data)){
            large = node -> data;
        }
        find_max(node -> right, start, stop);
    }
}

void maxima(int a, int b){
    find_max(root, a, b);
}
...
void main(){
    ...
    while(ch){
        ...
        printf("Enter 5 to find largest b/w two nodes.\n");
